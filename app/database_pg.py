...
class PgCursorWrapper:
    ...
    def execute(self, query, params=None):
        ...
        # Translate SQLite datetime() to PostgreSQL equivalents
        query = re.sub(r"datetime\('now'\)", "NOW()", query)
        query = re.sub(r"datetime\('now',\s*'([+-]?\d+)\s+(minute|minutes|hour|hours|day|days)'\)", r"NOW() + INTERVAL '\1 \2'", query)
        query = re.sub(r"datetime\('now',\s*'-(\d+)\s+(minute|minutes|hour|hours|day|days)'\)", r"NOW() - INTERVAL '\1 \2'", query)
        query = re.sub(r"datetime\('now',\s*'start of day'\)", "DATE_TRUNC('day', NOW())", query)
        query = re.sub(r"strftime\('%Y-%m-%dT%H:%M:00Z',\s*(\w+)\)", r"TO_CHAR(\1, 'YYYY-MM-DD\"T\"HH24:MI:00\"Z\"')", query)
...
class DatabaseManager:
    ...
    def get_connection(self):
        ...
        if self.db_path == ":memory:":
            ...
            if not hasattr(self, "_mem_conn"):
                self._mem_conn = psycopg2.connect(self.db_path)
                self._mem_pass
            conn = self._mem_conn
            try:
                yield conn
    ...
    def store_otp(self, user_id: int, otp_code: str, ttl_seconds: int = 60) -> str:
        ...
        Invalidates all previous unused OTPs for this user before storing
        the new one. Returns the OTP code.
        """
    ...
    def verify_otp(self, user_id: int, otp_code: str) -> bool:
        ...
        Returns True if the code is valid, not expired, and not yet used.
    ...
    def anonymize_user(self, user_id: int) -> None:
        ...
        with self.get_connection() as conn:
            conn.execute(
                "UPDATE users SET username = ?, email = ?, is_active = FALSE WHERE user_id = ?",
    ...
    def get_user_stats(self, user_id: int) -> Dict:
        ...
        with self.get_connection() as conn:
            ...
            cursor.execute(
                ...
                FROM auth_events 
                WHERE user_id = %s AND event_type = 'anomaly' 
