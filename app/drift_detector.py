"""Behavioral drift detection using rolling windows and statistical tests.

This module compares current behavioral feature distributions to baseline
reference distributions and returns drift scores for adaptive authentication.
"""
...
class BehavioralDriftDetector:
    ...
    def __init__(
        self, window_size: int = 100, alpha: float = 0.05, min_samples: int = 30
    ):
    ...
    def set_reference_baseline(
        self, keystroke_features: List[Dict], mouse_features: List[Dict]
    ):
    ...
    def _has_sufficient_samples(self) -> bool:
        ...
        return (
            ...
            or len(self.mouse_window) >= self.min_samples
        )
    ...
    def _check_for_drift(self):
        ...
        if self.reference_keystroke and len(self.keystroke_window) >= self.min_samples:
            keystroke_drift = self._detect_distribution_drift(
                list(self.keystroke_window), self.reference_keystroke, "keystroke"
            )
        ...
        if self.reference_mouse and len(self.mouse_window) >= self.min_samples:
            mouse_drift = self._detect_distribution_drift(
                list(self.mouse_window), self.reference_mouse, "mouse"
            )
    ...
    def _detect_distribution_drift(
        self, current_samples: List[Dict], reference_stats: Dict, data_type: str
    ) -> float:
    ...
    def _calculate_feature_drift(
        self, current_stats: Dict, reference_stats: Dict, feature_name: str
    ) -> float:
    ...
    def _detect_variance_change(
        self, current_stats: Dict, reference_stats: Dict
    ) -> float:
    ...
    def perform_statistical_tests(
        ...
        try:
            ...
            logger.exception(
                "Statistical drift tests failed for feature: %s", feature_name
            )
            results["error"] = str(e)
    ...
    def get_drift_analysis(self) -> Dict:
        ...
        analysis = {
            ...
            "last_drift_time": self.last_drift_time.isoformat()
    ...
    def _get_detailed_feature_analysis(self) -> Dict:
        ...
        if self.reference_keystroke and len(self.keystroke_window) >= self.min_samples:
            current_keystroke = self._calculate_feature_statistics(
                ...
                if feature_name in current_keystroke:
                    ...
                    keystroke_drift[feature_name] = {
                        ...
                        "mean_change_percent": float(
                            abs(
                                current_keystroke[feature_name]["mean"]
                                - self.reference_keystroke[feature_name]["mean"]
                            )
                            / max(self.reference_keystroke[feature_name]["mean"], 1e-6)
                            * 100
                        ),
                    }
            ...
            for feature_name in self.reference_mouse.keys():
                if feature_name in current_mouse:
                    ...
                    mouse_drift[feature_name] = {
                        ...
                        "mean_change_percent": float(
                            abs(
                                current_mouse[feature_name]["mean"]
                                - self.reference_mouse[feature_name]["mean"]
                            )
                            / max(self.reference_mouse[feature_name]["mean"], 1e-6)
                            * 100
                        ),
                    }
    ...
    def should_retrain(self) -> bool:
        ...
        if self.last_drift_time:
            ...
            if time_since_drift < timedelta(
                minutes=30
            ):  # Minimum time between retraining
                return False
    ...
    def get_adaptation_suggestions(self) -> List[str]:
        ...
        if "keystroke" in analysis:
            ...
            if "typing_speed_wpm" in keystroke_features:
                speed_change = keystroke_features["typing_speed_wpm"][
                    "mean_change_percent"
                ]
                if speed_change > 20:
                    suggestions.append(
                        "Significant typing speed change detected - consider stress or fatigue factors"
                    )
            ...
            timing_drift = [
                keystroke_features.get(f, {}).get("drift_score", 0)
                for f in timing_features
            ]

            if np.mean(timing_drift) > 0.4:
                suggestions.append(
                    "Typing rhythm patterns have changed - possible injury or adaptation"
                )
        ...
        if "mouse" in analysis:
            ...
            if "movement_efficiency" in mouse_features:
                efficiency_change = mouse_features["movement_efficiency"][
                    "mean_change_percent"
                ]
            ...
            motion_drift = [
                mouse_features.get(f, {}).get("drift_score", 0) for f in motion_features
            ]
        ...
        if self.drift_score > 0.6:
            suggestions.append(
                "Significant behavioral changes detected - recommend model retraining"
            )
        elif self.drift_score > 0.4:
            suggestions.append(
                "Moderate behavioral drift - monitor closely and consider gradual adaptation"
            )
