"""Behavioral feature extraction for keystroke and mouse biometrics.

The extractor produces a fixed-size feature vector (38 features) to keep model
input dimensions stable across training and inference.
"""
...
class BehavioralFeatureExtractor:
    ...
    def __init__(self, window_size: int = 30):
        ...
        self.MOUSE_FEATURES = [
            ...
            "direction_change_variance",  # 2
            "click_duration_mean",
            "click_duration_std",  # 2
            "left_click_ratio",
            "right_click_ratio",  # 2
            "inter_click_mean",
            "inter_click_std",  # 2
        ...
        ), f"Feature count mismatch! Got {self.FEATURE_COUNT}, expected 38"

        logger.info(
            f"BehavioralFeatureExtractor initialized with {self.FEATURE_COUNT} features"
        )
    ...
    def extract_keystroke_features(self, keystroke_data: List[Dict]) -> Dict:
        ...
        try:
            ...
            for event in keystroke_data:
                ...
                hold_time = (
                    event.get("hold_time")
                    or event.get("holdTime")
                    or event.get("duration")
                )
                flight_time = (
                    event.get("flight_time")
                    or event.get("flightTime")
                    or event.get("interval")
                )
                timestamp = event.get("timestamp") or event.get("time")

                if (
                    hold_time is not None
                    and isinstance(hold_time, (int, float))
                    and hold_time > 0
                ):
                    hold_times.append(hold_time)

                if (
                    flight_time is not None
                    and isinstance(flight_time, (int, float))
                    and flight_time > 0
                ):
    ...
    def _extract_timing_stats(self, prefix: str, times: List[float]) -> Dict:
        ...
        try:
            ...
            stats[f"{prefix}_std"] = (
                float(np.std(times_array)) if len(times_array) > 1 else 0.0
            )
    ...
    def _extract_speed_features(
        self, keystroke_data: List[Dict], timestamps: List[float]
    ) -> Dict:
        ...
        try:
            ...
            if window_size > 1:
                ...
                for i in range(0, len(keystroke_data) - window_size + 1, window_size):
                    ...
                    window_timestamps = [
                        event.get("timestamp", 0) for event in window_data
                    ]
    ...
    def _extract_rhythm_features(
        ...
        try:
            ...
            if len(inter_key_intervals) > 0:
                ...
                if mean_interval > 0:
                    features["rhythm_consistency"] = 1.0 / (
                        1.0 + (std_interval / mean_interval)
                    )
                else:
                    features["rhythm_consistency"] = 0.0
    ...
    def _extract_consistency_features(self, keystroke_data: List[Dict]) -> Dict:
        ...
        try:
            ...
            if len(hold_times) > 1:
                ...
                features["hold_time_cv"] = (
                    std_hold / mean_hold if mean_hold > 0 else 0.0
                )
            else:
            ...
            if len(flight_times) > 1:
                ...
                features["flight_time_cv"] = (
                    std_flight / mean_flight if mean_flight > 0 else 0.0
                )
            else:
            ...
            if digraphs:
                all_digraph_times = [
                    time for times in digraphs.values() for time in times
                ]
                if all_digraph_times:
                    features["digraph_consistency"] = 1.0 / (
                        1.0 + np.std(all_digraph_times)
                    )
                else:
    ...
    def _extract_click_features(self, mouse_data: List[Dict]) -> Dict:
        ...
        try:
            clicks = [
                event
                for event in mouse_data
                if event.get("type") == "click" or event.get("button") is not None
            ]
            ...
            if clicks:
                ...
                if click_durations:
                    ...
                    features["click_duration_std"] = (
                        float(np.std(click_durations))
                        if len(click_durations) > 1
                        else 0.0
                    )
                else:
                ...
                if len(clicks) > 1:
                    ...
                    if inter_click_intervals:
                        features["inter_click_mean"] = float(
                            np.mean(inter_click_intervals)
                        )
                        features["inter_click_std"] = (
                            float(np.std(inter_click_intervals))
                            if len(inter_click_intervals) > 1
                            else 0.0
                        )
                    else:
        ...
        try:
            ...
            if curvatures:
                ...
                features["curvature_std"] = (
                    float(np.std(curvatures)) if len(curvatures) > 1 else 0.0
                )
            else:
                features["curvature_mean"] = 0.0
                features["curvature_std"] = 0.0
            ...
            if len(directions) > 1:
                ...
                if direction_changes:
                    ...
                    features["direction_change_variance"] = float(
                        np.var(direction_changes)
                    )
                else:
                    features["avg_direction_change"] = 0.0
                    features["direction_change_variance"] = 0.0
    ...
    def _extract_behavioral_patterns(self, mouse_data: List[Dict]) -> Dict:
        ...
        try:
            # Dwell time analysis
            hover_events = [
                event for event in mouse_data if event.get("type") == "hover"
            ]
            if hover_events:
                dwell_times = [
            ...
            if len(positions) > 10:
                ...
                features["movement_area"] = (max(x_coords) - min(x_coords)) * (
        ...
        for feature_name in self.KEYSTROKE_FEATURES:
            if feature_name in features:
                ...
                if isinstance(value, (int, float)) and not (
                    ...
                    normalized[feature_name] = self._get_default_keystroke_value(
                        feature_name
                    )
            else:
                normalized[feature_name] = self._get_default_keystroke_value(
        ...
        for feature_name in self.MOUSE_FEATURES:
            if feature_name in features:
                ...
                if isinstance(value, (int, float)) and not (
                    ...
                    normalized[feature_name] = self._get_default_mouse_value(
                        feature_name
                    )
            else:
    ...
    def _get_default_mouse_value(self, feature_name: str) -> float:
        ...
        defaults = {
            ...
            "curvature_mean": 0.3,
            "curvature_std": 0.2,
            "avg_direction_change": 0.5,
            "direction_change_variance": 0.3,
            "click_duration_mean": 100.0,
            "click_duration_std": 30.0,
            "left_click_ratio": 0.8,
            "right_click_ratio": 0.2,
            "inter_click_mean": 1000.0,
            "inter_click_std": 200.0,
            "dwell_time_mean": 500.0,
            "movement_area": 10000.0,
            "movement_centrality": 300.0,
            "velocity_smoothness": 0.8,
        }
        return defaults.get(feature_name, 0.0)
    ...
    def _get_empty_keystroke_features(self) -> Dict:
        ...
        return {
            name: self._get_default_keystroke_value(name)
    ...
    def _get_empty_mouse_features(self) -> Dict:
        ...
        return {
            name: self._get_default_mouse_value(name) for name in self.MOUSE_FEATURES
        }

    def get_combined_features(
        self, keystroke_features: Dict, mouse_features: Dict
    ) -> Dict:
        ...
        assert (
            len(combined) == self.FEATURE_COUNT
        ), f"Combined features count mismatch: {len(combined)} != {self.FEATURE_COUNT}"
    ...
    def fix_feature_dimensions(self, features: Dict) -> Dict:
        ...
        for feature_name in self.KEYSTROKE_FEATURES:
            if feature_name in features:
                ...
                fixed_features[feature_name] = self._get_default_keystroke_value(
                    feature_name
                )
        ...
        for feature_name in self.MOUSE_FEATURES:
            if feature_name in features:
                ...
                fixed_features[feature_name] = self._get_default_mouse_value(
                    feature_name
                )
    ...
    def _calculate_trend(values: List[float]) -> float:
        ...
        try:
            ...
            logger.exception("Failed to calculate trend for values sequence")
            return 0.0
    ...
    def _calculate_smoothness(values: List[float]) -> float:
        ...
        try:
            ...
            logger.exception("Failed to calculate smoothness for values sequence")
            return 1.0
    ...
    def _calculate_curvature(
        ...
        try:
            ...
            logger.exception("Failed to calculate curvature for point triplet")
            return 0.0
